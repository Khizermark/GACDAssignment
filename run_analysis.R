## Download and processing of UCI HAR Dataset, producing a tidy dataset with the mean
...
file <- "project.zip"
download.file(fileUrl, destfile=paste("data", file, sep="/"), method="auto")
unzip(paste("data", file, sep="/"), exdir="data")
data_dir <- "UCI HAR Dataset"
dateDownloaded <- date()
dateDownloaded

## importing labels
labels <- read.table(paste("data", "UCI HAR Dataset", "activity_labels.txt", sep="/"), col.names=c("labelcode","label"))
...
## import training set
training_folder <- paste("data","UCI HAR Dataset", "train", sep="/")
...
## import test set
test_folder <- paste("data", "UCI HAR Dataset", "test", sep="/")
...
write.table(tidy, file="SD_means.txt", quote=FALSE, row.names=FALSE, sep="\t")
